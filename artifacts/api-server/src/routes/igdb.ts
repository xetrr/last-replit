...
export const igdbSearchGame: RequestHandler = async (req, res) => {
  ...
  res.json({ data: normaliseGame(data[0]) });
};

export const igdbGetById: RequestHandler = async (req, res) => {
...
export const igdbStatus: RequestHandler = async (_req, res) => {
  ...
  const token = await getAccessToken();
  res.json({ configured, connected: !!token });
};
